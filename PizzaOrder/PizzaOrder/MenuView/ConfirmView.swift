...
struct ConfirmView: View {
    ...
    fileprivate func extractedFunc() -> Stepper<Text> {
        return Stepper(value: $quantity, in:1...10){
    ...
    var body: some View {
        VStack{
            ...
            SelectedImageView(image: "\(menuID)_250w")
                .padding(10)
//                .onTapGesture(count: 2, perform: { //First way to dismiss
//                    self.isPresented = false
//                })
                .gesture( //Second way to dismiss
                    TapGesture(count: 2)
                        .onEnded {
                            self.isPresented = false
                        }
